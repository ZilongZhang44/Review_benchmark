...
###Random
RAN<- function(dat = deng.dat,n_feature = 2000){
  set.seed(1234567)
  deng_ran <- deng.dat[sample(rownames(deng.dat),n_feature),]
...
####HVG
HVG<- function(dat = deng.dat,n_feature = 2000){
  set.seed(1234567)
  seurat_deng <- CreateSeuratObject(counts = deng.dat)
  seurat_deng <- FindVariableFeatures(seurat_deng,nfeatures = n_feature)
  seurat_hvg<- deng.dat[which(VariableFeatures(seurat_deng)%in% rownames(deng.dat)),]
...
###HEG
HEG<- function(dat = deng.dat,n_feature = 2000){
  set.seed(1234567)
  ep <- apply(deng.dat,1,mean)
  top_heg <- as.matrix(head(ep,n_feature))
  deng_heg <- deng.dat[rownames(top_heg),]
...
GINI<- function(dat=deng.dat,n_feature=2000){
  set.seed(1234567)
  
  gini = apply(as.data.frame(deng.dat), 1, function(x){calcul.gini(as.numeric(x)) } )    #theoretically, gini have very low chance to have a 1 value
  GiniIndex = as.data.frame(cbind(1:dim(deng.dat)[1], gini))
  top_gini <- rownames(head(GiniIndex[order(GiniIndex[,2]),],n_feature))
  deng_gini <- deng.dat[top_gini,]
...
#####m3DROP
M3 <- function(dat = deng.dat, mt_threshold=0.05){
  set.seed(1234567)
  keep <- rownames(M3DropFeatureSelection(deng.dat,mt_method="none",mt_threshold=mt_threshold,suppress.plot=T))
  deng_m3 <-deng.dat[keep,]
