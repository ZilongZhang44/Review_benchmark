...
###Random
RAN<- function(dat = li.dat,n_feature = 2000){
  set.seed(1234567)
  li_ran <- li.dat[sample(rownames(li.dat),n_feature),]
...
####HVG
HVG<- function(dat = li.dat,n_feature = 2000){
  set.seed(1234567)
  seurat_li <- CreateSeuratObject(counts = li.dat)
  seurat_li <- FindVariableFeatures(seurat_li,nfeatures = n_feature)
  seurat_hvg<- li.dat[which(VariableFeatures(seurat_li)%in% rownames(li.dat)),]
...
###HEG
HEG<- function(dat = li.dat,n_feature = 2000){
  set.seed(1234567)
  ep <- apply(li.dat,1,mean)
  top_heg <- as.matrix(head(ep,n_feature))
  li_heg <- li.dat[rownames(top_heg),]
...
GINI<- function(dat=li.dat,n_feature=2000){
  set.seed(1234567)
  
  gini = apply(as.data.frame(li.dat), 1, function(x){calcul.gini(as.numeric(x)) } )    #theoretically, gini have very low chance to have a 1 value
  GiniIndex = as.data.frame(cbind(1:dim(li.dat)[1], gini))
  top_gini <- rownames(head(GiniIndex[order(GiniIndex[,2]),],n_feature))
  li_gini <- li.dat[top_gini,]
...
#####m3DROP
M3 <- function(dat = li.dat, mt_threshold=0.05){
  set.seed(1234567)
  keep <- rownames(M3DropFeatureSelection(li.dat,mt_method="none",mt_threshold=mt_threshold,suppress.plot=T))
  li_m3 <-li.dat[keep,]
